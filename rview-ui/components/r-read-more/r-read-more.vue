<template>
  <view class="">
    <view class="r-content" :class="[elId]" :style="{ height: isLongContent && !showMore ? showHeight + 'rpx' : 'auto' }">
      <slot></slot>
    </view>
    <view @tap="toggleReadMore" v-if="isLongContent" class="r-showmore-wrap" :class="{ 'r-show-more': showMore }"
      :style="[innerShadowStyle]">
      <text class="r-readmore-btn" :style="{
				fontSize: fontSize + 'rpx',
				color: color
			}">
        {{ showMore ? openText : closeText }}
      </text>
      <r-icon :color="color" :size="fontSize" class="r-icon" :name="showMore ? 'arrow-up' : 'arrow-down'"></r-icon>
    </view>
  </view>
</template>
...
<script>
   ...
   * @example <r-read-more><rich-text :nodes="content"></rich-text></r-read-more>
   */
  export default {
    name: "r-read-more",
    props: {
    ...
    methods: {
      init() {
        this.$rGetRect('.' + this.elId).then(res => {
