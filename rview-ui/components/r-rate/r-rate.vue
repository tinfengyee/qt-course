<template>
  <view class="r-rate" :id="elId" @touchmove.stop.prevent="touchMove">
    <view class="r-star-wrap" v-for="(item, index) in count" :key="index" :class="[elClass]">
      <r-icon :name="activeIndex > index ? activeIcon : inactiveIcon" @click="click(index + 1, $event)" :color="activeIndex > index ? activeColor : inactiveColor"
        :style="{
					fontSize: size + 'rpx',
...
<script>
   ...
   * @example <r-rate :count="count" :current="2"></r-rate>
   */
  export default {
    name: 'r-rate',
    props: {
    ...
    methods: {
      ...
      getElRectById() {
        ...
        this.$r.getRect('#' + this.elId).then(res => {
      ...
      getElRectByClass() {
        // uView封装的获取节点的方法，详见文档
        this.$r.getRect('.' + this.elClass).then(res => {
