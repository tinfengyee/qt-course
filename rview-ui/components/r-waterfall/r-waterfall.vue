<template>
	<view class="r-waterfall">
		<view id="r-left-column" class="r-column"><slot name="left" :leftList="leftList"></slot></view>
		<view id="r-right-column" class="r-column"><slot name="right" :rightList="rightList"></slot></view>
	</view>
</template>
 ...
 * @example <r-waterfall :flowList="flowList"></r-waterfall>
 */
export default {
	name: "r-waterfall",
	props: {
	...
	methods: {
		async splitData() {
			...
			let leftRect = await this.$rGetRect('#r-left-column');
			let rightRect = await this.$rGetRect('#r-right-column');
